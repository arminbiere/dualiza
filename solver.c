...
static Queue *
queue (Solver * solver, Var * v)
{
  if (v->type == RELEVANT_VARIABLE)
    return &solver->queue.relevant;
  if (v->type == IRRELEVANT_VARIABLE)
    return &solver->queue.irrelevant;
  if (v->type == PRIMAL_VARIABLE)
    return &solver->queue.primal;
...
static const char *
queue_type (Solver * s, Queue * q)
{
  if (q == &s->queue.relevant)
    return "relevant";
  if (q == &s->queue.irrelevant)
    return "irrelevant";
  if (q == &s->queue.primal)
    return "primal";
...
static void
update_queue (Solver * solver, Queue * q, Var * v)
{
  ...
  if (v)
    ...
    SOG ("empty %s queue", queue_type (solver, q));
#endif
}

static void
enqueue (Solver * solver, Var * v)
{
...
static void
dequeue (Solver * solver, Var * v)
{
  Queue *q = queue (solver, v);
  SOG ("%s dequeue variable %d stamp %ld",
       type (v), var2idx (solver, v), v->stamp);
  if (v->prev)
    assert (v->prev->next == v), v->prev->next = v->next;
  else
    assert (q->first == v), q->first = v->next;
  if (v->next)
    assert (v->next->prev == v), v->next->prev = v->prev;
  else
    assert (q->last == v), q->last = v->prev;
  if (q->search == v)
    {
      Var *u = v->next;
      if (!u)
...
static void
push_frame (Solver * solver, int decision)
{
  Frame f;
  f.decision = decision;
  f.seen = f.flipped = 0;
...
static Frame *
frame_at_level (Solver * solver, int level)
{
  assert (0 <= level);
...
static Frame *
last_frame (Solver * solver)
{
  return frame_at_level (solver, solver->level);
}

static int
decision_at_level (Solver * solver, int level)
{
  assert (level > 0);
...
static void
init_reduce_limit (Solver * solver)
{
  ...
  SOG ("initial reduce learned limit %ld", solver->limit.reduce.learned);
}

static void
init_restart_limit (Solver * solver)
{
  solver->limit.restart.conflicts = MAX (options.restartint, 1);
  SOG ("initial restart conflict limit %ld", solver->limit.restart.conflicts);
}

static void
set_subsumed_learned_limit (Solver * solver)
{
...
void
limit_number_of_partial_models (Solver * solver, long limit)
{
  assert (limit > 0);
...
static void
enable_model_printing (Solver * solver, Name name)
{
  solver->name = name;
...
Solver *
new_solver (CNF * primal, IntStack * shared, IntStack * relevant, CNF * dual)
{
  check_options_fixed ();
  assert (primal);
  Solver *solver;
  ...
  if (dual)
    {
      SOG ("new solver: %ld primal, %ld dual clauses, %ld shared variables",
	   ...
	   (long) COUNT (dual->clauses), (long) COUNT (*shared));
  ...
  else
    {
      SOG ("new solver: %ld clauses, %ld shared variables",
	   (long) COUNT (primal->clauses), (long) COUNT (*shared));
      solver->cnf.dual = 0;
  ...
  SOG ("%d primal or shared variables", solver->max_primal_or_shared_var);
  ...
  if (options.relevant)
  ...
  else
    {
      ...
      if (dual)
	reason = "in dual mode";
      else if (count_irrelevant)
	reason = "with irrelevant variables";
      else if (count_primal)
	reason = "with primal variables";
      else
	reason = 0;
      if (reason)
	{
	  msg (1, "split on relevant variables after first model (%s)",
	       reason);
...
static int
is_relevant_var (Var * v)
{
  return v->type == RELEVANT_VARIABLE;
}

static int
is_irrelevant_var (Var * v)
{
...
static int
is_primal_or_shared_var (Var * v)
{
  return v->type != DUAL_VARIABLE;
}

static int
is_dual_or_shared_var (Var * v)
{
  return v->type != PRIMAL_VARIABLE;
}

static Clauses *
primal_occs (Solver * solver, int lit)
{
...
static Clauses *
dual_occs (Solver * solver, int lit)
{
...
static CNF *
cnf (Solver * solver, Clause * c)
{
  return c->dual ? solver->cnf.dual : solver->cnf.primal;
}

static int
lit_sign (int literal)
{
  return literal < 0 ? -1 : 1;
}

static void
dec_unassigned (Solver * solver, Var * v)
{
  if (is_primal_or_shared_var (v))
    {
...
static void
inc_unassigned (Solver * solver, Var * v)
{
  if (is_primal_or_shared_var (v))
    {
      solver->unassigned_primal_and_shared_variables++;
...
static void
assign (Solver * solver, int lit, Clause * reason)
{
  int idx = abs (lit);
  ...
  if (!reason)
    ...
    SOGCLS (reason, "%s assign %d reason", type (v), lit);
  assert (!v->val);
  if (lit < 0)
    v->val = v->phase = -1;
  else
...
static Decision
decision_type (Solver * solver, int lit)
{
...
static void
adjust_next (Solver * solver, int next)
{
  assert (0 <= next), assert (next <= COUNT (solver->trail));
  if (next < solver->next.primal)
    solver->next.primal = next;
  if (next < solver->next.dual)
    solver->next.dual = next;
}
...
static void
inc_solver_level (Solver * solver)
{
  solver->level++;
...
static void
inc_decision_levels (Solver * solver)
{
...
static void
dec_decision_levels (Solver * solver)
{
  assert (solver->level > 0);
  Frame *f = last_frame (solver);
  assert (!f->flipped);
...
static void
inc_flipped_levels (Solver * solver)
{
  assert (solver->level > 0);
  Frame *f = last_frame (solver);
  assert (f->flipped);
  ...
  SOG ("incremented number flipped levels to %d", solver->num_flipped_levels);
  SOG ("new last flipped level %d", solver->last_flipped_level);
}

static void
dec_flipped_levels (Solver * solver)
{
  assert (solver->level > 0);
  Frame *f = last_frame (solver);
  assert (f->flipped);
  ...
  SOG ("decremented number flipped levels to %d", solver->num_flipped_levels);
  SOG ("new last flipped level %d", solver->last_flipped_level);
}
...
static void
assume_decision (Solver * solver, int lit)
{
...
static void
dec_level (Solver * solver)
{
  assert (solver->level > 0);
  Frame *f = last_frame (solver);
  if (f->flipped)
    dec_flipped_levels (solver);
  else
...
static void
flip_decision (Solver * solver, long *rule)
{
  stats.flipped++;
  assert (solver->level > 0);
  Frame *f = last_frame (solver);
  ...
  if (rule == &rules.BP1F)
    RULE1 (BP1F, -decision);
  else if (rule == &rules.BP0F)
    RULE1 (BP0F, -decision);
  else if (rule == &rules.BN0F)
    RULE1 (BN0F, -decision);
  else
    SOG ("WARNING UNMATCHED RULE in '%s'", __FUNCTION__);
...
static Decision
unassign (Solver * solver, int lit)
{
  Var *v = var (solver, lit);
...
static void
connect_primal_literal (Solver * solver, Clause * c, int lit)
{
  assert (!c->dual);
...
static void
connect_primal_clause (Solver * solver, Clause * c)
{
  assert (!c->dual);
  assert (c->size > 1);
...
static int
connect_primal_cnf (Solver * solver)
{
  assert (!solver->level);
  CNF *cnf = solver->cnf.primal;
  Clauses *clauses = &cnf->clauses;
  SOG ("connecting %ld primal clauses to solver", (long) COUNT (*clauses));
...
static void
connect_dual_literal (Solver * solver, Clause * c, int lit)
{
  assert (c->dual);
  SOGCLS (c, "connecting literal %d to", lit);
...
static void
connect_dual_clause (Solver * solver, Clause * c)
{
  assert (c->dual);
  assert (c->size > 1);
  ...
  for (int i = 0; i < n; i++)
    {
      ...
      if (!tmp)
	continue;
      if (!is_relevant_var (var (solver, lit)))
	continue;
      if (!first)
	fputc (' ', stdout);
      if (tmp < 0)
...
static void
print_discount (Solver * solver, Frame * f)
{
  assert (solver->model_printing_enabled);
...
static int
model_limit_reached (Solver * solver)
{
  const long limit = solver->limit.models.count;
  const int res = (stats.models.counted >= limit);
  if (res)
    msg (1, "reached partial models limit %ld", limit);
  return res;
}

static int
sort_clause (Solver * solver)
{
...
#ifndef NLOG
  if (size > 0)
    SOG ("first sorted literal %d", lits[0]);
  if (size > 1)
    SOG ("second sorted literal %d", lits[1]);
#endif
  return size;
}

static int
remaining_variables (Solver * solver)
{
  return solver->max_var - solver->fixed - stats.eliminated;
}

static void
inc_model_report_limit (Solver * solver)
{
	...
	int precision, int percent_suffix, int flush_right, double value)
{
  ...
  if (++ENTRIES == num_report_header_lines)
    ENTRIES = 0;
}

static void
report (Solver * solver, int verbosity, char ch)
{
...
new_model (Solver * solver)
{
  ...
  if (stats.models.counted == 1)
    first_model (solver);
  if (solver->model_printing_enabled)
    print_model (solver);
...
static int
last_model (Solver * solver, int *unassigned_ptr)
{
...
static int
connect_dual_cnf (Solver * solver)
{
  assert (!solver->level);
  if (!solver->dual_solving_enabled)
    return 1;
...
static void
report_iterating (Solver * solver)
{
  if (!solver->found_new_fixed_variable)
    return;
...
static Clause *
primal_propagate (Solver * solver)
{
...
static Clause *
dual_force (Solver * solver, Clause * c, int lit)
{
...
static Clause *
dual_propagate_units (Solver * solver)
{
...
static Clause *
dual_propagate_trail (Solver * solver)
{
  ...
  while (!res && solver->next.dual < COUNT (solver->trail))
    {
      ...
      solver->next.dual++;
      assert (val (solver, lit) > 0);
      if (is_primal_var (var (solver, lit)))
	continue;
...
static int
dual_propagating (Solver * solver)
{
  if (!solver->dual_solving_enabled)
    return 0;
...
{
  if (!dual_propagating (solver))
    return 0;
  SOG ("dual propagation");
  check_primal_propagated (solver);
  Clause *res = 0;
  if (!solver->last_decision_level)
    res = dual_propagate_units (solver);
  if (!res)
    res = dual_propagate_trail (solver);
  report_iterating (solver);
  return res;
}

static Var *
next_queue (Solver * solver, Queue * queue)
{
...
next_decision (Solver * solver)
{
  ...
  if (!res || !solver->split_on_relevant_first)
	...
	res = primal;
      if (!res)
	res = next_queue (solver, &solver->queue.dual);
    }
  assert (res);
...
static int
is_unit_clause (Solver * solver, Clause * c)
{
  int unit = 0;
  for (int i = 0; i < c->size; i++)
    {
...
static void
check_no_empty_clause (Solver * solver)
{
  if (!options.check)
    return;
...
static int
is_empty_clause (Solver * solver, Clause * c)
{
  for (int i = 0; i < c->size; i++)
    {
      const int lit = c->literals[i];
...
static void
check_no_unit_clause (Solver * solver)
{
  if (!options.check)
    return;
  for (int dual = 0; dual <= 1; dual++)
...
static void
check_all_relevant_variables_assigned (Solver * solver)
{
  for (int idx = 1; idx <= solver->max_var; idx++)
    {
      Var *v = var (solver, idx);
...
static void
check_dual_propagated (Solver * solver)
{
  if (!dual_propagating (solver))
    return;
...
static void
mark_literal (Solver * solver, int lit)
{
...
static void
unmark_literal (Solver * solver, int lit)
{
...
static void
mark_clause (Solver * solver, Clause * c)
{
  for (int i = 0; i < c->size; i++)
...
static void
unmark_clause (Solver * solver, Clause * c)
{
  for (int i = 0; i < c->size; i++)
...
static int
marked_literal (Solver * solver, int lit)
{
  Var *v = var (solver, lit);
...
static void
flush_primal_garbage_occurrences (Solver * solver)
{
  long flushed = 0;
  for (int idx = 1; idx <= solver->max_primal_or_shared_var; idx++)
    for (int sign = -1; sign <= 1; sign += 2)
...
static void
flush_dual_garbage_occurrences (Solver * solver)
{
  long flushed = 0;
  for (int idx = 1; idx <= solver->max_var; idx++)
    {
      if (!is_dual_or_shared_var (var (solver, idx)))
	continue;
      for (int sign = -1; sign <= 1; sign += 2)
	{
	  int lit = sign * idx;
...
static void
subsume_learned (Solver * solver, Clause * c)
{
  if (!options.subsume)
    return;
  if (!options.sublearned)
    return;
  if (c->size <= 1)
    return;
  assert (!c->redundant);
  ...
  if (stats.subsumed <= solver->limit.subsumed)
    return;
...
static void
adjust_next_to_trail (Solver * solver)
{
  adjust_next (solver, COUNT (solver->trail));
}

static void
register_new_fixed_variable (Solver * solver)
{
  if (solver->last_decision_level > 0)
    return;
...
static void
add_decision_blocking_clause (Solver * solver, long *rule)
{
  assert (solver->level > 0);
  Frame *f = last_frame (solver);
  ...
  for (;;)
    {
      ...
      if (unassign (solver, other) != UNDECIDED)
	dec_level (solver);
      if (other == first)
	break;
    }
...
static int
blocking_clause_size (Solver * solver, int target_level)
{
...
static int
blocking (Solver * solver, int target_level)
{
  if (!options.block)
    return 0;
  ...
  int res = (size <= limit);
  if (res)
    SOG ("blocking clause size %d below limit %d", size, limit);
  else
    SOG ("blocking clause size %d would exceed limit %d", size, limit);
  return res;
}
...
static int
is_relevant_decision_level (Solver * solver, int level)
{
  if (!level)
    return 0;
  Frame *f = frame_at_level (solver, level);
...
static int
is_primal_satisfied_no_log (Solver * solver)
{
  if (solver->next.primal < COUNT (solver->trail))
    return 0;
  return !solver->unassigned_primal_and_shared_variables;
}

static void
check_primal_satisfied (Solver * solver)
{
  assert (is_primal_satisfied_no_log (solver));
}

static int
is_primal_satisfied (Solver * solver)
{
  int res = is_primal_satisfied_no_log (solver);
  if (res)
    SOG ("all primal and shared variables assigned");
  return res;
}
...
static int
last_relevant_decision (Solver * solver)
{
...
static void
backtrack (Solver * solver, int level)
{
...
static void
initialize_count (Solver * solver, int level, int counted)
{
  Frame *f = frame_at_level (solver, level);
...
static int
backtrack_primal_satisfied (Solver * solver)
{
  ...
  if (blocking (solver, level))
    ...
    backtrack_primal_satisfied_flip (solver, level, counted);
  return 1;
}
...
static int
resolve_literal (Solver * solver, int lit)
{
  Var *v = var (solver, lit);
  if (!v->level)
    return 0;
...
static int
resolve_clause (Solver * solver, Clause * c)
{
  assert (c);
  SOGCLS (c, "resolving");
  c->recent = 1;
...
static int
cmp_seen (const void *p, const void *q)
{
  Var *u = *(Var **) p, *v = *(Var **) q;
  if (u->stamp < v->stamp)
    return -1;
...
static void
bump_variable (Solver * solver, Var * v)
{
  stats.bumped++;
...
static void
bump_reason_clause_literals (Solver * solver, Clause * c)
{
  if (!c)
    return;
  for (int i = 0; i < c->size; i++)
...
static void
bump_all_reason_literals (Solver * solver)
{
...
static void
bump_seen (Solver * solver)
{
  if (options.bump > 1)
    bump_all_reason_literals (solver);
  sort_seen (solver);
  for (Var ** p = solver->seen.start; p < solver->seen.top; p++)
...
static void
update_ema (Solver * solver, double *ema, double res, double n, double alpha)
{
  assert (n > 0);
...
static Clause *
learn_primal_clause (Solver * solver, int glue, int level)
{
  stats.learned++;
  const int size = COUNT (solver->clause);
  ...
  if (size > 1)
    connect_primal_clause (solver, res);
  CLEAR (solver->clause);
  return res;
}

static void
discount (Solver * solver)
{
  assert (options.discount);
  Frame *f = solver->frames.start + solver->level;
  assert (f->flipped);
  if (is_zero_number (f->count))
    return;
  assert (f->counted > 0);
...
static int
resolve_primal_conflict (Solver * solver, Clause * conflict)
{
...
static long
flipped_levels_above (Solver * solver, int level)
{
  long res = 0;
  int flipped = 0;
  for (int i = level + 1; i <= solver->level; i++)
...
static int
analyze_primal_conflict (Solver * solver, Clause * conflict)
{
  ...
  if (!options.learn)
  ...
  else
    {
      ...
      SOG ("discounting %ld levels above back-jump level %d", discount,
	   level);
      if (discount > 0)
...
static int
analyze_dual_conflict (Solver * solver, Clause * conflict)
{
  if (restart_after_first_model_to_split_on_relevant_first (solver))
    {
      force_splitting_on_relevant_first (solver);
...
static int
reducing (Solver * solver)
{
  return primal_reducing (solver) || dual_reducing (solver);
}

static void
inc_reduce_limit (Solver * solver)
{
...
static int
mark_satisfied_as_garbage (Solver * solver, Clause * c)
{
  assert (!c->garbage);
  for (int i = 0; i < c->size; i++)
    {
...
static void
reduce_primal (Solver * solver)
{
  if (!primal_reducing (solver))
    return;
  ...
  for (Clause ** p = primal->clauses.start; p < primal->clauses.top; p++)
    {
      ...
      if (c->garbage)
	continue;
      if (simplify && mark_satisfied_as_garbage (solver, c))
	continue;
      if (c->active)
	continue;
      if (!c->redundant)
	continue;
      if (c->size <= options.keepsize)
	continue;
      if (c->glue <= options.keepglue)
	continue;
...
static void
reduce_dual (Solver * solver)
{
  if (!dual_reducing (solver))
    return;
  ...
  const int simplify =
    solver->dual_or_shared_fixed > solver->limit.reduce.dual_or_shared_fixed;
  ...
  for (Clause ** p = dual->clauses.start; p < dual->clauses.top; p++)
    {
      Clause *c = *p;
      if (c->garbage)
	continue;
  ...
  solver->limit.reduce.dual_or_shared_fixed = solver->dual_or_shared_fixed;
}
...
static void
inc_restart_limit (Solver * solver)
{
  const int inc = MAX (options.restartint, 1);
  solver->limit.restart.conflicts = stats.conflicts.primal + inc;
  SOG ("new restart conflicts limit %ld", solver->limit.restart.conflicts);
}

static int
restarting (Solver * solver)
{
  if (!options.restart)
    return 0;
  if (solver->last_flipped_level)
    return 0;
  if (!solver->level)
    return 0;
  if (stats.conflicts.primal <= solver->limit.restart.conflicts)
    return 0;
...
static void
force_splitting_on_relevant_first (Solver * solver)
{
  assert (!solver->split_on_relevant_first);
...
solve (Solver * solver)
{
  if (model_limit_reached (solver))
    return;
  if (!connect_primal_cnf (solver))
    return;
  if (primal_propagate (solver))
    return;
  if (is_primal_satisfied (solver))
    {
      (void) new_model (solver);
      return;
    }
  if (!connect_dual_cnf (solver))
    {
      (void) new_model (solver);
      return;
    }
  for (;;)
    {
      ...
      if (conflict)
	{
	  if (!analyze_primal_conflict (solver, conflict))
	    return;
	}
      else if (is_primal_satisfied (solver))
	{
	  if (!backtrack_primal_satisfied (solver))
	    return;
	}
      else
	{
	  ...
	  if (conflict)
	    {
	      if (!analyze_dual_conflict (solver, conflict))
		return;
	    }
...
void
primal_count (Number models, Solver * solver)
{
  assert (!solver->dual_solving_enabled);
  solve (solver);
...
void
dual_count (Number models, Solver * solver)
{
  assert (solver->dual_solving_enabled);
  solve (solver);
  copy_number (models, solver->count);
...
void
primal_enumerate (Solver * solver, Name name)
{
  assert (!solver->dual_solving_enabled);
...
void
dual_enumerate (Solver * solver, Name name)
{
  assert (solver->dual_solving_enabled);
  enable_model_printing (solver, name);
  solve (solver);
...
void
print_trail (Solver * solver)
{
  for (int level = 0; level <= solver->level; level++)
    {
      int start = frame_at_level (solver, level)->trail, end;
      if (level == solver->level)
	end = (int) COUNT (solver->trail);
      else
	end = frame_at_level (solver, level + 1)->trail;
