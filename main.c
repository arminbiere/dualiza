...
static void
check_options (const char *output_name)
{
...
{
  if (formula)
    msg (1, "formula printing mode due to%s", FORMULA);
  if (dimacs)
    msg (1, "DIMACS printing mode due to%s", DIMACS);
  if (aiger)
    msg (1, "AIGER printing mode due to%s", AIGER);
  if (sat)
    msg (1, "satisfiability checking mode due to%s", SAT);
  if (tautology)
    msg (1, "tautology checking mode due to%s", TAUTOLOGY);
  if (enumerate)
    msg (1, "enumeration mode due to%s", ENUMERATE);
  if (!printing && !checking && !enumerate)
    {
      if (counting)
	msg (1, "counting mode due to%s", COUNTING);
      else
	msg (1, "default counting mode"), counting = 1;
    }
...
static int
is_non_negative_number_string (const char *s)
{
  const char *p = s;
  char ch = *p++;
  if (!isdigit (ch))
...
static long
parse_non_negative_number (const char *s)
{
  long res = 0;
  if (s[0] == '0' && s[1])
  ...
  for (const char *p = s; *p; p++)
    {
      if (LONG_MAX / 10 < res)
	die ("number '%s' really too large", s);
      res *= 10;
      const int digit = *p - '0';
      if (LONG_MAX - digit < res)
	die ("number '%s' too large", s);
      res += digit;
    }
  return res;
}
...
static void
setup_input (const char *input_name)
{
  if (input_name)
    input = open_new_reader (input_name);
  else
    input = new_reader_from_stdin ();
  msg (1, "reading from '%s'", input->name);
}

static void
parse (const char *input_name)
{
  ...
  if (info == FORMULA)
      ...
      if (relevant_strs)
	{
	  ...
	  NEW (relevant);
	  INIT (*relevant);
	  for (char **p = relevant_strs->start; p != relevant_strs->top; p++)
	    {
	      Symbol *s = find_symbol (symbols, *p);
	      if (!s)
		die ("relevant symbol '%s' not in formula", *p);
  ...
  if (relevant)
    msg (1, "PROJECTING ON %" PRz " RELEVANT VARIABLES", COUNT (*relevant));
}
  ...
  if (negate)
    {
      assert (!checking);
      if (printing)
	msg (1, "generating dual circuit for printing");
      else if (enumerate)
	msg (1, "generating dual circuit for enumeration");
      else
	assert (counting), msg (1, "generating dual circuit for counting");
    }
  else
    {
      if (checking)
	...
	{
	  msg (1, "using primal circuit for non-negated printing");
	  return;
	}
      else if (enumerate)
	{
      ...
      else
	{
	  ...
	  if (bdd)
	    ...
	    msg (1, "generating dual circuit for counting with SAT engine");
	}
    }
...
print (const char *output_name)
{
  ...
  msg (1, "finished writing '%s'", output_name ? output_name : "<stdout>");
}
  ...
  if (bdd)
    {
      ...
      BDD *b = simulate_primal ();
      if (negate)
	printf ("ALL FALSIFYING ASSIGNMENTS\n");
      else
	printf ("ALL SATISFYING ASSIGNMENTS\n");
      fflush (stdout);
  ...
  else if (options.primal)
    {
      ...
      if (limited)
	limit_number_of_partial_models (solver, limit);
      if (negate)
	printf ("ALL FALSIFYING ASSIGNMENTS\n");
      else
	printf ("ALL SATISFYING ASSIGNMENTS\n");
...
static void
init_relevant (unsigned num_inputs)
{
  NEW (relevant);
  INIT (*relevant);
  ...
  if (bdd)
    {
      ...
      BDD *b = simulate_primal ();
      if (negate)
	printf ("NUMBER FALSIFYING ASSIGNMENTS\n");
      else
      ...
      if (num_inputs)
	{
	  ...
	  init_number (n);
	  if (!relevant)
	    init_relevant (num_inputs);
	  count_bdd (n, b, relevant);
	  double time = process_time ();
	  double delta = time - start;
	  msg (1,
	       "BDD solution counting of primal circuit in %.3f seconds",
	       delta);
	  if (options.print)
	    println_number (n), fflush (stdout);
	  clear_number (n);
	}
      else
	printf ("%d\n", is_true_bdd (b));
...
reset ()
{
  ...
  if (relevant_ints)
    {
      RELEASE (*relevant_ints);
      DELETE (relevant_ints);
    }
  if (relevant_strs)
    {
...
static void
setup_messages (const char *output_name)
{
  if (!printing)
    return;
  if (output_name)
...
sort_and_flush_relevant_ints ()
{
  ...
  qsort (relevant_ints->start, n, sizeof (int), cmp_ints);
  size_t i = 0;
  int prev = -1;
...
static int
cmp_strs (const void *p, const void *q)
{
...
sort_and_flush_relevant_strs ()
{
  ...
  qsort (relevant_strs->start, n, sizeof (char *), cmp_strs);
  size_t i = 0;
  const char *prev = 0;
  for (size_t j = 0; j < n; j++)
    {
...
parse_relevant_ints (char *arg)
{
  ...
  for (char *p = arg, *next; p < end; p = next + 1)
      ...
      for (const char *q = p; q < next; q++)
	{
	  ...
	  if (INT_MAX / 10 < res)
	  INVALID:die ("invalid integer '%s' in argument to '-r'",
		 p);
	  res *= 10;
	  int digit = *q - '0';
	  if (digits++ && !res && !digit)
...
static void
parse_relevant_variables (const char *arg)
{
  if (!*arg)
    die ("empty argument to '-r'");
  if (*arg == ',')
    die ("leading ',' in argument to '-r'");
  if (arg[strlen (arg) - 1] == ',')
    die ("trailing ',' in argument to '-r'");
...
main (int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++)
	...
	aiger = +1;
      else if (!strcmp (argv[i], "--dimacs"))
	dimacs = +1;
      else if (!strcmp (argv[i], "--negate"))
	negate = +1;
      else if (!strcmp (argv[i], "--sat"))
	sat = +1;
      else if (!strcmp (argv[i], "--tautology"))
	tautology = +1;
      else if (!strcmp (argv[i], "--enumerate"))
	enumerate = +1;
      else if (!strcmp (argv[i], "--counting"))
	counting = +1;
      else if (!strcmp (argv[i], "--bdd"))
	bdd = +1;
	...
	{
	  if (++i == argc)
	    die ("output file argument to '-o' missing'");
	  if (output_name)
	...
	{
	  if (limited)
	    die ("multiple limits '%ld' and '%s'", limit, argv[i]);
...
#ifndef NLOG
  if (options.logging)
    options.verbosity = INT_MAX;
#endif
  check_options (output_name);
  setup_messages (output_name);
  msg (1, "Dualiza #SAT Solver");
  msg (1,
       "Copyright (C) 2017-2021 Armin Biere Johannes Kepler University Linz");
  print_version ();
  init_mode ();
  set_signal_handlers ();
  print_options ();
