...
Gate *
decode_literal (Circuit * circuit, int lit)
{
  assert (lit), assert (lit != INT_MIN);
  int idx = abs (lit) - 1;
  if (idx >= COUNT (circuit->inputs))
    return 0;
...
int
encode_input (Circuit * circuit, Gate * g)
{
  int sign = SIGN (g);
  if (sign)
    g = STRIP (g);
...
void
print_dimacs_encoding_to_file (Circuit * c, FILE * file)
{
...
void
print_dimacs_encoding (Circuit * circuit)
{
  print_dimacs_encoding_to_file (circuit, stdout);
}

static Encoder *
new_encoder (Circuit * circuit, CNF * cnf)
{
  Encoder *res;
  NEW (res);
  res->circuit = circuit;
...
static int
marked_lit (Encoder * e, int lit)
{
  int idx = abs (lit);
  if (idx >= COUNT (e->marks))
    return 0;
...
static void
unmark_clause_literals (Encoder * e)
{
...
static void
encode_unary (Encoder * e, int lit)
{
...
static void
encode_binary (Encoder * e, int a, int b)
{
  assert (EMPTY (e->clause));
  PUSH (e->clause, a);
  PUSH (e->clause, b);
...
static void
encode_ternary (Encoder * e, int a, int b, int c)
{
  assert (EMPTY (e->clause));
  PUSH (e->clause, a);
  PUSH (e->clause, b);
...
static int
map_input (Gate * g, int input, Encoder * e)
{
  return map_gate (get_gate_input (g, input), e);
}

static void
encode_false (Gate * g, Encoder * e)
{
...
encode_and (Gate * g, Encoder * e)
{
  ...
  assert (EMPTY (e->clause));
  if (options.polarity && g->pos && !g->neg)
    return;
  PUSH (e->clause, lit);
  for (int i = 0; i < n; i++)
...
encode_gate (Gate * g, Encoder * e)
{
  switch (g->op)
    ...
    case XNOR_OPERATOR:
      encode_xnor (g, e);
      break;
    case INPUT_OPERATOR:	/* UNREACHABLE */
      break;
    }
}

static int
encode_inputs (Encoder * e)
{
  Circuit *c = e->circuit;
...
static int
encode_gates (Encoder * encoder, Gate * g, int idx)
{
  g = STRIP (g);
  if (g->code)
    return idx;
...
static int
encode_root (Encoder * encoder, Gate * g, int idx)
{
  ...
  if (!h->code)
    idx = encode_gates (encoder, h, idx);
  encode_unary (encoder, map_gate (g, encoder));
  return idx;
}

static int
encode_roots (Encoder * encoder, Gate * g, int idx)
{
  ...
  if (!sign && g->op == AND_OPERATOR)
    ...
    idx = encode_root (encoder, g, idx);
  return idx;
}

static void
reset_code_root_fields_of_circuit (Circuit * c)
{
...
void
encode_circuit (Circuit * circuit, CNF * cnf)
{
...
void
encode_circuits (Circuit * c, Circuit * d, CNF * f, CNF * g)
{
...
void
get_encoded_inputs (Circuit * c, IntStack * s)
{
  for (Gate ** p = c->inputs.start; p < c->inputs.top; p++)
