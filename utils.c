...
int
has_suffix (const char *s, const char *t)
{
...
static FILE *
open_pipe (int write, const char *fmt, const char *arg)
{
#ifdef __MINGW32__
  ...
  DEALLOC (cmd, bytes);
  if (!res)
    die ("'popen (\"%s\", \"%s\")' failed", cmd, type);
  return res;
#endif
}

FILE *
read_pipe (const char *fmt, const char *arg)
{
  return open_pipe (0, fmt, arg);
}

FILE *
write_pipe (const char *fmt, const char *arg)
{
...
int
is_space_character (int ch)
{
  return ch == ' ' || ch == '\t' || ch == '\n' || ch == '\n' || ch == '\r';
}

void *
first_non_zero_byte (void *p, size_t size)
{
...
const char *
make_character_printable_as_string (char ch)
{
  static char buffer[16];
  if (isprint (ch))
    sprintf (buffer, "'%c'", ch);
  else
    sprintf (buffer, "code 0x%d", ch);
  return buffer;
}
