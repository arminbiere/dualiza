...
static int
is_start_of_basic_expression_character (int ch)
{
  if (ch == '(' || ch == '!')
    return 1;
  if (ch == '0' || ch == '1')
...
static Gate *
parse_basic (Parser * parser)
{
  Coo ch = next_non_white_space_char (parser->reader);
  Gate *res = 0;
  if (ch.code == EOF)
    ...
    res = NOT (new_false_gate (parser->circuit));
  else if (ch.code == '0')
    res = new_false_gate (parser->circuit);
    ...
    parse_error_at (parser->reader, ch, "expected basic expression");
  return res;
}
...
static Gate *
parse_implies_or_iff (Parser * parser)
{
...
Circuit *
parse_formula (Reader * reader, Symbols * symbols)
{
  Parser parser;
