...
static int
is_relevant_variable_string (const char *s)
{
  if (!s)
    return 0;
  const char *p = s;
...
parse_dimacs (Reader * r, Symbols * symbols, IntStack ** relevant_ptr)
{
  ...
  while ((ch = next_char (r)).code == 'c')
	...
	{
	  if (ch.code == EOF)
	    parse_error (r, ch, "unexpected end-of-file in header comment");
	  if (ch.code != '\r')
	    PUSH (comment, ch.code);
	}
      PUSH (comment, 0);
      if (options.project && is_relevant_variable_string (comment.start))
	{
  ...
  while (isdigit ((ch = next_char (r)).code))
    {
      if (!s)
	parse_error (r, ch, "invalid number of variables");
      if (INT_MAX / 10 < s)
  ...
  while (isdigit ((ch = next_char (r)).code))
    {
      if (!t)
	parse_error (r, ch, "invalid number of clauses");
      if (INT_MAX / 10 < t)
  ...
  while (ch.code != '\n')
    if (!is_space_character (ch.code))
      parse_error (r, ch,
		   "non-space character before newline after 'p cnf %d %d'",
		   s, t);
    else
      ch = next_char (r);
  ...
  if (parsed_relevant_variables)
      ...
      for (size_t i = 0; i < n; i++)
	{
	  ...
	  if (seen[idx])
	    LOG ("ignoring duplicated relevant index '%d'", idx);
	  else
	    {
	      POKE (*relevant, j, idx);
	      seen[idx] = 1;
  ...
  for (int i = 0; i < s; i++)
    {
      ...
      g->symbol = symbol;
      LOG ("DIMACS variable %d connected to input gate %d with symbol '%s'",
  ...
  for (;;)
	...
	{
	  if (!EMPTY (clause))
	    parse_error (r, ch, "end-of-file without '0' after last clause");
	...
	{
	  if (!i)
	    parse_error (r, start, "invalid variable index number");
	  if (INT_MAX / 10 < i)
...
#ifndef NLOG
      if (EMPTY (clause))
	LOG ("start clause %ld", 1 + COUNT (clauses));
#endif
      if (i)
	{
